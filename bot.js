...
function hasExplicitAgeOrDateNearKeyword(textLower) {
  const dateOrAgePatterns = [
    ...
    // Полные периоды: 3 месяца, 7 дней, 1 год (с пробелами и без)
    ...
    // Сокращения без пробелов: 30д, 90d, 7m, 3y, 2w и т.д.
    /(?:^|[^0-9A-Za-zА-Яа-яЁё])\d+\+?\s*[а-яa-z](?=$|[^0-9A-Za-zА-Яа-яЁё])/iu,
    // "от Xд", "от 30д", "от 90d" и т.д.
    /\b(?:от|за|в течение|через)\s+\d+\+?\s*[а-яa-z]/iu,
    // Годы: 2023, 2024 (обычно с контекстом "в 2023")
    /(?:^|[^0-9A-Za-zА-Яа-яЁё])(?:с|в)\s*\d{4}(?=$|[^0-9A-Za-zА-Яа-яЁё])/iu,
    /(?:^|[^0-9A-Za-zА-Яа-яЁё])\d{4}(?=$|[^0-9A-Za-zА-Яа-яЁё])/iu,
    // Формат дата: 14/03, 14-03, 14.03
